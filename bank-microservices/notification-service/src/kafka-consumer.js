...
// ── Kafka event → Notification document mapping ──────────────────────────────
...
function buildNotification(topic, event) {
  ...
  switch (topic) {
    ...
    case 'transfer-completed':
      return { ...base, user_id: event.fromAccount, type: 'virement', title: 'Virement effectué', body: `Virement de ${event.amount} TND vers le compte ${event.toAccount} effectué avec succès.` };
    ...
    await consumer.run({
      eachMessage: async ({ topic, message }) => {
        try {
          ...
          console.log(`[notification-service] Événement Kafka reçu: ${topic} —`, event.event || '');

          const notif = buildNotification(topic, event);
          if (notif) {
            notifications.insert(notif);
            console.log(`[notification-service] Notification créée: "${notif.title}" pour ${notif.user_id}`);
          }
        } catch (e) {
